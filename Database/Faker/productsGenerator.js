...
const filename = argv.filename || '../csvFiles/generated-product.csv';

// Declare writeStream as a function which uses the filename variable and tells fs to open a writable stream
const writeStream = fs.createWriteStream(filename);

// Declare a function that generates a row / entry of fake data for the table
const createProductEntry = () => (`"${faker.commerce.productName()}","${faker.lorem.sentence()}","${faker.lorem.paragraph()}","${faker.commerce.department()}",${faker.random.number({ min: 1, max: 999 })}\n`);
